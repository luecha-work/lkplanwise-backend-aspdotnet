...
MSSQL_SCAFFOLD_CMD = dotnet ef dbcontext scaffold "Server=localhost;Database=principalsdb;User ID=sa;Password=P@ssw0rd123;TrustServerCertificate=True" Microsoft.EntityFrameworkCore.SqlServer -o $(OUTPUT_FOLDER)
POSTGRES_SCAFFOLD_CMD = dotnet ef dbcontext scaffold "Host=localhost;Database=principalsdb;Username=planwiseroot;Password=P@ssw0rd123" Npgsql.EntityFrameworkCore.PostgreSQL -o $(OUTPUT_FOLDER)

network:
	docker network create bank-network

postgres:
	docker run --name planwise-postgres-container -p 5432:5432 -e POSTGRES_USER=planwiseroot -e POSTGRES_PASSWORD=P@ssw0rd123 -d postgres
